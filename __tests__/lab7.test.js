describe('Basic user flow for Website', () => {
  ...
  // Each it.skip() call is a separate test
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const textJsonValue = await text.jsonValue();
      if (textJsonValue === 'Add to Cart') {
        await button.click();
      }
    }
    ...
    const cartCountValueText = await cartCountValue.jsonValue();
    expect(cartCountValueText).toBe('20');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let allButtonsSayRemoveFromCart = true;
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      const buttonText = await button.getProperty('innerText');
      const buttonTextJsonValue = await buttonText.jsonValue();

      if (buttonTextJsonValue !== 'Remove from Cart') {
        allButtonsSayRemoveFromCart = false;
      }

    }

    expect(allButtonsSayRemoveFromCart).toBe(true);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      if (textJsonValue === 'Remove from Cart') {
        await button.click();
      }
    }
    ...
    const cartCountValue = await cartCount.getProperty('innerText');
    const cartCountValueText = await cartCountValue.jsonValue();
    expect(cartCountValueText).toBe('0');
    console.log('here2');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let allButtonsSayAddToCart = true;
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      if (buttonTextJsonValue !== 'Add to Cart') {
        allButtonsSayAddToCart = false;
      }

    }

    expect(allButtonsSayAddToCart).toBe(true);
    const cartCount = await page.$('#cart-count');
    const cartCountValue = await cartCount.getProperty('innerText');
    const cartCountValueText = await cartCountValue.jsonValue();
    expect(cartCountValueText).toBe('0');
